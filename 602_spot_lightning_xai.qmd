...
if os.path.exists(filename) and not overwrite:
    (S, fun_control, design_control,
    surrogate_control, optimizer_control) = load_result(PREFIX)
else:
...
```{python}
#| label: print_results
res = S.print_results(print_screen=True)
```
...
import pprint
config = get_tuned_architecture(S, fun_control)
...
from spotpython.hyperparameters.values import get_tuned_architecture
config = get_tuned_architecture(S, fun_control, force_minX=True)
pprint.pprint(config)
```
