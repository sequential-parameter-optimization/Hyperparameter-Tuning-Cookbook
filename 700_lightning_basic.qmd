...
## Starter Example: Simple Neural Network
...
class LightningBasic(L.LightningModule):
    ...
    **kwargs):
        ...
        hidden_sizes = get_hidden_sizes(_L_in=self._L_in, l1=l1, n=4)
...
Before we can call the `Trainer` to fit, validate, and test the model, we need to create the `DataLoaders`.
Therefore, we have to spefify the `batch_size`.
