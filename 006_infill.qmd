...
Forrester emphasizes that effective infill criteria are designed to combine both exploitation and exploration.

### Expected Improvement (EI)

**Expected Improvement (EI)** is one of the most influential and widely-used infill criteria, particularly within **Bayesian optimization**. Formalized by Jones (1998) and building upon the work of Mockus (1978), EI provides a mathematically elegant framework that naturally balances exploitation and exploration. Rather than simply picking the point with the best predicted value (pure exploitation) or the point with the highest uncertainty (pure exploration), EI asks a more nuanced question: "How much improvement over the current best solution can we *expect* to gain by evaluating the true function at a new point $x$?".
