...
An additional parameter, $\sigma^2$, is introduced to control the rate of decay. This parameter is referred to as the length scale or the characteristic length scale. The covariance function is then defined as 
...
def squared_euclidean_distance(point1, point2, sigma2):
    return np.sum(1/sigma2 * (np.array(point1) - np.array(point2))**2)
...
def build_Sigma(X, sigma2):
    ...
    for l in range(k):
        for i in range(n):
            for j in range(i, n):
                D[l, i, j] = 1/sigma2[l]*(X[i,l] - X[j,l])**2
    D = sum(D)
    D = D + D.T
    return exp(-D)  
```
...
Kriging basis functions (as introduced in @eq-Sigma) of the form 
$$
