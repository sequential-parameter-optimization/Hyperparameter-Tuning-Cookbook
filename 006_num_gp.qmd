...
The *Gaussian* kernel uses an additional parameter, $\sigma^2$, to control the rate of decay. This parameter is referred to as the length scale or the characteristic length scale. The covariance function is then defined as 
...
def squared_euclidean_distance(point1, point2, sigma2):
    return np.sum(1/(2*sigma2) * (np.array(point1) - np.array(point2))**2)
...
def visualize_inverse_exp_squared_distance_3d(x_max, y_max, sigma2, n):
    x = np.linspace(-x_max, x_max, n)
    y = np.linspace(-y_max, y_max, n)
    X, Y = np.meshgrid(x, y)
    ...
    for i in range(n):
        for j in range(n):
            Z[i, j] = inverse_exp_squared_distance((X[i, j], Y[i, j]), (0, 0), sigma2)
...
visualize_inverse_exp_squared_distance_3d(x_max=1, y_max=1, sigma2=np.array([1,3]), n=100)
```
...
* When $x$ and $x'$ are one $\sigma$ unit apart, the correlation is $\exp\left(-\sigma^2 / (2\sigma^2)\right) = \exp(-1/2) \approx 0.61$, i.e., a decent amount of correlation.
* $2\sigma$ apart means correlation $\exp(− 2^2 /2) \approx 0.14$, i.e., only slightly dependent.
* $4\sigma$ apart means correlation $\exp(− 4^2 /2) \approx 0.0003$, i.e., essentially independent for almost all practical purposes.
...
$$
are used in a method known as Kriging. Note, $x^{(i)}$ denotes the $k$-dim vector $x^{(i)}= (x_1^{(i)}, \ldots, x_k^{(i)})^T$.
...
### Generate samples from N($\mu$, $\Psi$)
