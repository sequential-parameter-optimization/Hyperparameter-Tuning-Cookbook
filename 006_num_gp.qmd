...
The following block of code generates five draws from a bivariate normal distribution with zero mean and covariance matrix $\Psi$. Because our input space is three dimensional, the results cannot be visualized in a simple way. The next examples will use a one-dimensional input space, so that our results can be easily visualized in 2d.
