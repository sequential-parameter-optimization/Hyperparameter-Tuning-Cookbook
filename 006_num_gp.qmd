...
```{python}
# generate 10 random numbers between from a uniform distribution
import numpy as np
...
You can compute the correlation matrix from a covariance matrix in Python using the numpy library. The correlation matrix is computed by dividing each element of the covariance matrix by the product of the standard deviations of the corresponding variables.
...
\Sigma(x, x') = \exp\{ - || x - x'||^2 \},
$$
where $x$ and $x'$ are two points in the $k$-dimensional input space and $\| \cdot \|$ denotes the Euclidean distance, i.e.,
$$
...
Especially in higher dimensions, an additional parameter, $\theta$, is introduced to control the rate of decay. This parameter is referred to as the length scale or the characteristic length scale. The covariance function is then defined as 
...
def squared_euclidean_distance(point1, point2, theta):
    return np.sum(theta*(np.array(point1) - np.array(point2))**2)

def inverse_exp_squared_distance(point1, point2, theta):
    return np.exp(-squared_euclidean_distance(point1, point2, theta))

def generate_grid(x_max, y_max, n):
    x = np.linspace(-x_max, x_max, n)
    y = np.linspace(-y_max, y_max, n)
    return [(xi, yi) for xi in x for yi in y]

def visualize_inverse_exp_squared_distance_3d(x_max, y_max, theta, n):
    x = np.linspace(-x_max, x_max, n)
    y = np.linspace(-y_max, y_max, n)
    X, Y = np.meshgrid(x, y)
    ...
    for i in range(n):
        for j in range(n):
            Z[i, j] = inverse_exp_squared_distance((X[i, j], Y[i, j]), (0, 0), theta)
    
    contour = plt.contour(X, Y, Z, levels = 10, cmap='viridis')
    plt.clabel(contour, inline=True, fontsize=8)
    plt.gca().set_aspect('equal', adjustable='box')  # Make x and y axis the same size
    plt.show()
```

```{python}
#| label: fig-exp2euclid2d
#| fig-cap: "Two-dim inverse exponentiated squared Euclidean distance"
visualize_inverse_exp_squared_distance_3d(x_max=1, y_max=1, theta=np.array([1,3]), n=100)
```
...
By default, the mean vector is initialized to zero. To match the number of samples, which is equivalent to the number of rows in the `X` and `Psi` matrices, the argument `zeros(n)` is used, where `n` represents the number of samples (here taken from the size of the matrix, e.g.,: `Psi.shape[0]`).

```{python}
rng = np.random.default_rng(seed=12345)
Y = rng.multivariate_normal(zeros(Psi.shape[0]), Psi, size = 1, check_valid="raise").reshape(-1,1)
Y.shape
```
...
```{python}
rng = np.random.default_rng(seed=12345)
Y = rng.multivariate_normal(zeros(Psi.shape[0]), Psi, size = 3, check_valid="raise")
plt.plot(X, Y.T)
plt.title("Realization of Random Functions under a GP prior.\n theta: {}".format(theta[0]))
plt.show()
```
...
y = [y_1, y_2, \ldots, y_n].
$$
We are interested in finding an expression for the predicted value at a new point $x$. We are going to interpret the observed responses as if they are generated by a stochastic process, which will be denoted as follows:
$$
Y = [Y(x_1), \ldots, Y(x_n) ]^T.
$$
The random process has a mean of $\mu$ (which is a (n,1)-dimensional vector).
...
Finally, $$ \Psi = \exp(-D)$$ is computed.  Next, we will demonstrate how this computation can be implemented in Python.
...
The following block of code generates 5 draws from a bivariate normal distribution with zero mean and covariance matrix $\Psi$. Because our input space is three dimensional, the results cannot be visualized in a simple way. The next examples will use a one-dimensional input space, so that our results can be easily visualized in 2d.
...
After the optimization, the correlation matrix is build with the optimized $\theta$ and $p$ values. This is best (most likely) Kriging model for the given data $y$. We will skip the optimization step here and use $\theta = 1$ and $p=2$.
...
The correlation matrix $\Psi$ is based on the pairwise squared distances between the input locations. Here we will use $n=8$ sample locations
* $\theta$ is set to 1.0
