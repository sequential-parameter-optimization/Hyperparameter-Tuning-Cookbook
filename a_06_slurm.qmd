...
Save the code in a file named `startSlurm.sh` and copy the file to the remote machine via `scp`, i.e., `scp startSlurm.sh user@144.33.22.1:`.
...
Save the code in a file named `startPython.py` and copy the file to the remote machine via `scp`, i.e., `scp startPython.py user@144.33.22.1:`.
...
The configuration is saved as  a pickle-file that contains the full information. In our example, the filename is `spot_light-user-model_experiment.pickle`.
...
#| eval: false
scp spot_light-user-model_experiment.pickle user@144.33.22.1:
```
