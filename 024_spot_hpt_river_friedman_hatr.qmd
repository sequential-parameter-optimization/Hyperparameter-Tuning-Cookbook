...
We will use the Friedman drift data set for this demonstration. The Friedman drift data set is a synthetic data set with 10 features and 1 target. It will be generated using the `river` library as shown below.
...
train_dataset = synth.FriedmanDrift(
   drift_type='gra',
   position=(n_train/4, n_train/2),
   seed=123
)

train = convert_to_df(train_dataset, n_total=n_train)
train.columns = [f"x{i}" for i in range(1, 11)] + [target_column]


test_dataset = synth.FriedmanDrift(
   drift_type='gra',
   position=(n_train/4, n_train/2),
   seed=123
)
test = convert_to_df(test_dataset, n_total=n_test)
...
dict = hyperdict().hyper_dict[coremodel]
...
from spotRiver.utils.data_conversion import convert_to_df
df = convert_to_df(dataset, n_total=n_samples)
```
