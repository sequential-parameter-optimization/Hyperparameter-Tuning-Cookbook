...
`spotpython` has several options to cope with noisy functions:
  ...
  3. `ocba_delta` is set to a value larger than 1 (here: 2). This means that the OCBA algorithm is used to allocate the computational budget optimally. An example is given in @sec-ocb-example.
