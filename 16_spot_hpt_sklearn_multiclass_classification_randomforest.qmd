...
# HPT: sklearn RandomForestClassifier VBDP Data {#sec-hpt-random-forest-classifier}
...
## Step 4: Specification of the Preprocessing Model {#sec-specification-of-preprocessing-model-16}
...
Since `oob_score` requires the `bootstrap` hyperparameter to `True`, we set the `oob_score` parameter to `False`. The `oob_score` is later discussed in @sec-oob-score-16.
...
### Preparing the SPOT Call {#sec-prepare-spot-call-16}
...
### The Objective Function {#sec-the-objective-function-16}
...
## Step 9: Tensorboard {#sec-tensorboard-16}
...
## Step 10: Results {#sec-results-tuning-16}
