...
# Use columns from 'type' to 'mpg' as predictors
...
* The following code should be unnecessary if the data frame names are set consistently (to be done!)

```{python}
df_z = df['ln_sales']
df_in_with_const = X_encoded_with_const
df_in = X_encoded
```
...
1. Check for NaN or infinite values in `df_in`.
2. Replace NaN and infinite values with the median of the respective column.
3. Remove constant or nearly constant columns.
4. Standardize the numerical predictors in `df_in` using `StandardScaler`.
5. Verify that `X_scaled` does not contain any NaN or infinite values.

```{python}
if df_in.isnull().any().any():
    print("NaNs detected in df_in. Filling with column medians.")
    df_in = df_in.fillna(df_in.median())

if np.isinf(df_in.values).any():
    print("Infinite values detected in df_in. Replacing with large finite values.")
    df_in = df_in.replace([np.inf, -np.inf], np.nan).fillna(df_in.median())

df_in = df_in.loc[:, df_in.std() > 1e-6]
...
For each variable, the ratio of the squared sums of correlations to the squared sums of partial correlations is computed. The formula for the KMO measure is:
...
```{python}
fa_temp = FactorAnalyzer(n_factors=X_scaled.shape[1], method="principal", rotation=None)
try:
...
```{python}
anz_fak = 10
n_factors = min(anz_fak, X_scaled.shape[1])
...
factor_loadings_df = pd.DataFrame(
    factor_loadings,
    index=df_in.columns,  # Original feature names
...
### Creating the Regression Model with Extracted Factors

```{python}
X_model = sm.add_constant(df_factors)
model_factors = sm.OLS(df_z, X_model).fit()
...
predictions_linear = model.predict(df_in_with_const)
...
mse_linear = mean_squared_error(df_z, predictions_linear)
rmse_linear = np.sqrt(mse_linear)

mse_factors = mean_squared_error(df_z, predictions_factors)
...
**Coefficients:** The coefficients in the `model_factors` regression model indicate the relationship between the target variable (`df_z`) and the extracted factors.

### Differences Compared to the Standard OLS Model

**Input Variables:**

- **OLS Model (`model`):** Uses the original variables (`df_in`) as predictors.
...
# Use the original input features (df_in) as predictors
X_original = df_in
y = df_z  # Target variable
