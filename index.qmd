...
@sec-hpt-sklearn-svc describes the hyperparameter tuning of a support vector classifier from `scikit-learn` [@scikit-learn] with `spotPython`.
@sec-sec-river-hpt illustrates the hyperparameter tuning of a Hoeffding Adaptive Tree Regressor from `river` [@river] with `spotPython`.
...
@sec-hpt-random-forest-classifier describes the hyperparameter tuning of a random forest classifier from `scikit-learn` [@scikit-learn] with `spotPython`.
@sec-hpt-sklearn-xgb-classifier-vbdp-data describes the hyperparameter tuning of a XGBoost classifier from `scikit-learn` [@scikit-learn] with `spotPython`.
@sec-hpt-sklearn-svc-vbdp-data describes the hyperparameter tuning of a support vector classifier from `scikit-learn` [@scikit-learn] with `spotPython`.
@sec-hpt-sklearn-knn-classifier-vbdp-data describes the hyperparameter tuning of a k-nearest neighbors classifier from `scikit-learn` [@scikit-learn] with `spotPython`.
